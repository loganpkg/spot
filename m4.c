 ...
 * Diversion -1 is index 10 which is discarded.
 */
#define NUM_DIVS 11
...
#define arg(n) ((n) <= m4->mc.active_arg ? m4->store->a + m4->mc.arg_i[n] : "")
...
struct macro_call {
    ...
    size_t arg_i[NUM_ARGS];     /* Name is arg 0. Only access elements <= active_arg. */
    size_t active_arg;
...
struct m4_info {
    int exit_val;               /* User requested exit value */
    struct entry **ht;
    int read_stdin;
    struct buf *input;
    struct buf *token;
...
int dumpdef(M4ptr m4)
{
    size_t i;
    struct entry *e;
    if (!m4->mc.active_arg || (m4->mc.active_arg == 1 && *arg(1) == '\0')) {
...
int main(int argc, char **argv)
{
    ...
    if (upsert(m4->ht, "paste", NULL, &paste))
        mgoto(clean_up);
    ...
    while (1) {
        if ((exit_val = m4->exit_val) != -1)    /* User requested exit */
            break;
